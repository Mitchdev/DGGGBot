...
exports.commandHandler = function(interaction) {
  ...
  interaction.editReply({content: (sorted.length > 0) ? `${sorted.map((m) => {
    ...
    return `${m.username} is a ${capitalize(m.roleName.replace('ROLE_', '').toLowerCase())} until ${secondsToDhms(time)} | **${m.gamble} gamble${m.gamble == 1 ? '' : 's'} left**`;
