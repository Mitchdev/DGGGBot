...
exports.commandHandler = async function(interaction, Discord) {
  ...
  if (interaction.options.get('user')) {
    const userData = triviaLeaderboard.find((p) => p.UserName === interaction.options.get('user').user.username);
    const userIndex = triviaLeaderboard.findIndex((p) => p.UserName === interaction.options.get('user').user.username);
    if (userData) {
    ...
    embed.addFields([{
      name: 'Username',
      value: triviaLeaderboard.slice(0, 10).map((p, i) => `${(i % 2 === 1) ? '**' : ''}#${i+1} ${p.UserName}${(i % 2 === 1) ? '**' : ''}`).join('\n'),
      inline: true,
    }, {
      name: 'Score per Game',
      value: triviaLeaderboard.slice(0, 10).map((p, i) => `${(i % 2 === 1) ? '**' : ''}${p.ScorePerGuess.toString()}${(i % 2 === 1) ? '**' : ''}`).join('\n'),
      inline: true,
    }, {
      name: 'Average Time',
      value: triviaLeaderboard.slice(0, 10).map((p, i) => `${(i % 2 === 1) ? '**' : ''}${p.AverageTime.toString()}${(i % 2 === 1) ? '**' : ''}`).join('\n'),
      inline: true,
    }]);
    embed.setFooter(`Page (1/${Math.ceil(triviaLeaderboard.length/10)})`);
