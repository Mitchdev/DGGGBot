          ...
          if (!err) {
            ...
            if (data?.data?.children != undefined) {
              ...
              if (filtered.length > 0) {
                  ...
                  if (filtered[0].data.secure_media) {
                    if (filtered[0].data.secure_media.oembed.thumbnail_url) {
                      ...
                      client.users.fetch(process.env.DEV_ID).then((devLog) => {
                        devLog.send({content: `**Reddit:** https://reddit.com${filtered[0].data.permalink}.json`});
                      });
                    }
                  }
                ...
                client.channels.resolve(feeds.list[feedIndex].channel).send({content: `**${filtered[0].data.title}** - ${filtered[0].data.subreddit_name_prefixed}\n${images.join('\n')}`});
                updateFeed();
              }
            }
          }
