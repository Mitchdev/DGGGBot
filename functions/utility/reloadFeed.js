module.exports = function(client) {
  reloadFeed = async function(channel) {
    const feedIndex = feeds.list.findIndex((feed) => feed.channel == channel);
    if (feedIndex >= 0) {
      if (feeds.list[feedIndex].subs.length > 0) {
        const data = await (await fetch(process.env.SUBREDDIT_HOT_API.replace('|subreddit|', feeds.list[feedIndex].subs[Math.floor(Math.random() * feeds.list[feedIndex].subs.length)]))).json();
