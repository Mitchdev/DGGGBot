    ...
    if (pos != undefined) {
      if (role.type == 'Gaming') {
        if (pos <= gamingRoles.length && pos > 0) {
          ...
          interaction.editReply({content: `Added ${role.name}`, ephemeral: true});
        } else {
          interaction.editReply({content: `Position out of range (1-${gamingRoles.length})`, ephemeral: true});
          return;
        }
      } else {
        if (pos <= generalRoles.length && pos > 0) {
          ...
          interaction.editReply({content: `Position out of range (1-${generalRoles.length})`, ephemeral: true});
          return;
        }
      }
    } else {
